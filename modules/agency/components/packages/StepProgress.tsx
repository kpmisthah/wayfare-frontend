    ...
    {steps.map((step, index) => {
      ...
      return (
        <div key={step.id} className="flex items-center">
          <div
            className={`w-12 h-12 rounded-full flex items-center justify-center border-2 transition-all duration-200 ${isActive
          ...
          {index < steps.length - 1 && (
            <div
              className={`hidden lg:block w-16 h-1 mx-4 rounded-full ${isCompleted ? "bg-green-300" : "bg-gray-200"
                }`}
            />
          )}
        </div>
